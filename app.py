...
def create_video_synced(text, audio_path, out_path, style='عادي', txt_color=None, bg_color=None):
    ...
    duration = float(audio_clip.duration)  # تأكد من أنها float
    ...
    if not words:
        words = [text]
    seg_duration = duration / len(words)  # float
...
# ==================================================
# 5. واجهة HTML (جميلة ومستجيبة)
# ==================================================
HTML_TEMPLATE = """
<!DOCTYPE html>
<html dir="rtl" lang="ar">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>مولد الفيديو الذكي - أغاني وفيديوهات متحركة</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Segoe UI',Tahoma;background:linear-gradient(135deg,#1e3c72,#2a5298);padding:20px}
.container{max-width:950px;margin:auto;background:#fff;border-radius:30px;overflow:hidden;box-shadow:0 20px 40px rgba(0,0,0,0.2)}
.header{background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;padding:35px;text-align:center}
.header h1{font-size:32px;margin-bottom:8px}
.content{padding:35px}
.form-group{margin-bottom:25px}
label{display:block;font-weight:bold;margin-bottom:10px;color:#1e293b}
textarea,select,input{width:100%;padding:14px;border:2px solid #e2e8f0;border-radius:16px;font-size:16px;transition:0.2s}
textarea:focus,select:focus,input:focus{border-color:#667eea;outline:none}
.row{display:grid;grid-template-columns:1fr 1fr;gap:20px}
button{width:100%;padding:16px;background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;border:none;border-radius:40px;font-size:18px;cursor:pointer;transition:0.2s}
...
.info{background:#e0f2fe;padding:18px;border-radius:20px;margin-top:25px;font-size:14px;text-align:center;color:#0369a1}
@media(max-width:650px){.row{grid-template-columns:1fr}}
</style>
</head>
<body>
<div class="container">
<div class="header"><h1>🎬 صانع الفيديو الذكي</h1><p>حوّل نصك إلى فيديو متحرك أو أغنية احترافية</p></div>
<div class="content">
<div class="form-group"><label>📝 النص (اكتب قصتك، أغنية، أو أي فكرة)</label><textarea id="text" rows="4" placeholder="مثال: في يوم من الأيام، كان هناك طفل يحب الرسم..."></textarea></div>
<div class="row">
<div class="form-group"><label>🎨 اختر النمط البصري</label><select id="style"><option value="عادي">📄 عادي (كلاسيكي)</option><option value="كرتوني">🎈 كرتوني (مرح)</option><option value="سينمائي">🎞️ سينمائي (فيلمي)</option><option value="أغنية">🎵 أغنية (ألوان دافئة)</option></select></div>
<div class="form-group"><label>🔊 تحويل النص إلى صوت</label><select id="use_tts"><option value="true">✅ نعم (يقرأ النص تلقائياً)</option><option value="false">❌ لا (فيديو صامت)</option></select></div>
</div>
<div class="row">
<div class="form-group"><label>🎨 لون الخلفية (اختياري)</label><select id="bg_color"><option value="">-- افتراضي حسب النمط --</option><option value="أسود">أسود</option><option value="أبيض">أبيض</option><option value="أحمر">أحمر</option><option value="أزرق">أزرق</option><option value="أخضر">أخضر</option><option value="أصفر">أصفر</option></select></div>
<div class="form-group"><label>✏️ لون النص (اختياري)</label><select id="txt_color"><option value="">-- افتراضي حسب النمط --</option><option value="أبيض">أبيض</option><option value="أسود">أسود</option><option value="أصفر">أصفر</option><option value="أحمر">أحمر</option></select></div>
</div>
<div class="row">
<div class="form-group"><label>⏱️ مدة الفيديو (ثواني) - للنص الثابت فقط</label><input type="number" id="duration" value="6" min="3" max="45" step="1"></div>
<div class="form-group"><label>🎬 نوع التزامن مع الصوت</label><select id="sync_type"><option value="static">📌 نص ثابت (أسرع)</option><option value="synced">🎤 نص متحرك مع الصوت (أغنية)</option></select></div>
</div>
<button id="generateBtn">✨ إنشاء الفيديو الآن ✨</button>
<div class="loading" id="loading"><div class="spinner"></div><p>جاري إنشاء الفيديو... قد يستغرق 10-30 ثانية حسب طول النص</p></div>
<div class="result" id="result"></div>
<div class="error" id="error"></div>
<div class="info">💡 هذا الموقع يعمل بدون أي مفاتيح API خارجية. يستخدم تقنيات مفتوحة المصدر (MoviePy + gTTS). جرب كتابة أغنية قصيرة أو قصة وستحصل على فيديو فوري.</div>
</div></div>
<script>
...
function showResult(url) { resultDiv.innerHTML = `<p>✅ تم إنشاء الفيديو بنجاح!</p><a href="${url}" download style="display:inline-block;background:#10b981;color:#fff;padding:12px 24px;border-radius:40px;text-decoration:none;margin:15px 0">📥 تحميل الفيديو (MP4)</a><br><video width="100%" controls src="${url}"></video>`; resultDiv.style.display = 'block'; }
